...
class Server:
    ...
    def __init__(
        self, client_manager: ClientManager, save_fd: bool, load_weights_dir: str, save_weights_dir: str,strategy: Optional[Strategy] = None
    ) -> None:
        self._client_manager: ClientManager = client_manager
        self.save_fd = save_fd
        self.load_weights_dir: str = load_weights_dir
        self.save_weights_dir: str = save_weights_dir
    ...
    def evaluate(
        ...
        if not client_instructions:
            ...
            return None
        log(DEBUG, "evaluate: strategy sampled %s clients", len(client_instructions),)
    ...
    def fit_round(self, rnd: int) -> Optional[Weights]:
        ...
        log(INFO, 'Start aggregating their results')
        return self.strategy.aggregate_fit(self.save_fd, self.save_weights_dir, rnd, results, failures)
    ...
    def _get_initial_weights(self) -> Weights:
        # Load weights 
        try:
            weights = pickle.load(open(self.load_weights_dir, 'rb'))
            log(DEBUG, 'Weights from the last federated learning process successfully loaded')
        except:
            log(ERROR,'Error when loading weights, leave...')
            sys.exit()
